...
import (
	...
	_su "SUSE-Manager-Tools-V2/internal/systemUpdate"
...
func main() {
	...
	flag.StringVar(&params.ConfigFile, "i", filepath.Join(path, ConfigFile), "Configuration file containing the needed configuration. Default: configsm.yaml in the directory where updateServer has been started.")
	flag.Parse()

	fmt.Println(params.Server)
	_su.Execute(params)
